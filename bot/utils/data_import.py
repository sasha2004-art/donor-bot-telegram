...
async def import_data_from_file(
    ...
    if not all(col in df.columns for col in required_cols):
        raise ValueError(
            f"Ошибка: в файле отсутствуют обязательные колонки ({', '.join(required_cols)})."
        )
    ...
    for index, row in df.iterrows():
        ...
        university = (
            "НИЯУ МИФИ"
            if "сотрудник" in str(study_group).lower()
            or (study_group and study_group != "-")
            else "Внешний донор"
        )
        ...
        user_data = {
            ...
            "study_group": (
                study_group
                if university == "НИЯУ МИФИ" and faculty != "Сотрудник"
                else None
            ),
            "role": "student",
        }
        ...
        total_donations = (
            donations_gavrilov if pd.notna(donations_gavrilov) else 0
