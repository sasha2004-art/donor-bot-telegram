...
class Text:
    ...
    @staticmethod
    def format_location_link(
        text: str, latitude: float | None, longitude: float | None
    ) -> str:
    ...
    SWITCH_TO_DONOR_VIEW = (
        "✅ Вы переключились в режим донора. Здесь доступны все студенческие функции."
    )
    ...
    UNKNOWN_COMMAND = (
        "Я не понимаю эту команду. Пожалуйста, используйте кнопки меню для навигации."
    )
    ...
    DONATION_HISTORY_ITEM = "▪️ {date}: {type}\n" #, <b>+{points} баллов</b>
    ...
    MERCH_PURCHASE_INSUFFICIENT_FUNDS = (
        "Недостаточно баллов. Нужно {price}, у вас {points}."
    )
    ...
    QR_EVENT_CAPTION = (
        "Покажите этот QR-код волонтёру после донации для подтверждения."
    )
    VOLUNTEER_MENU_HEADER = "⭐ <b>Меню волонтёра</b>"
    VOLUNTEER_SEND_QR_PROMPT = "Отправьте мне фотографию с QR-кодом донора."
    QR_READ_ERROR = (
        "❌ QR-код не распознан или имеет неверный формат. Попробуйте снова."
    )
    QR_INVALID_DATA_ERROR = "❌ QR-код содержит некорректные данные."
    QR_DB_LOOKUP_ERROR = (
        "❌ Ошибка: Пользователь или мероприятие из QR-кода не найдены в базе."
    )
    QR_DONOR_NOT_REGISTERED_ERROR = (
        "❌ Ошибка: Донор <b>{donor_name}</b> не зарегистрирован на это мероприятие."
    )
    QR_WRONG_DAY_ERROR = (
        "❌ Ошибка: Этот QR-код для мероприятия, которое проходит не сегодня."
    )
    ...
    VOLUNTEER_CONFIRMATION_ERROR = (
        "❌ Произошла ошибка. Пожалуйста, отсканируйте QR-код заново."
    )
    ...
    DONATION_CONFIRM_CRITICAL_ERROR = (
        "Произошла критическая ошибка при подтверждении донации: {error}"
    )
    ...
    EVENT_CREATE_STEP_6_POINTS = (
        "Выбран тип: {donation_type}\n\nШаг 6/7: Введите количество баллов за донацию:"
    )
    ...
    EVENT_EDIT_PROMPT = (
        "Вы редактируете: <b>{event_name}</b>\n\nКакое поле хотите изменить?"
    )
    ...
    MAILING_PHOTO_RECEIVED = (
        "Фото получено.\n\n<b>Шаг 3/3:</b> Выберите аудиторию для рассылки:"
    )
    MAILING_VIDEO_RECEIVED = (
        "Видео получено.\n\n<b>Шаг 3/3:</b> Выберите аудиторию для рассылки:"
    )
    ...
    MERCH_CREATE_PHOTO_ID_ERROR = (
        "❌ Произошла ошибка: ID фото не был сохранен. Начните заново."
    )
    ...
    ADMIN_COMPLETE_ORDER_ADMIN_ID_ERROR = (
        "Ошибка: не удалось идентифицировать администратора."
    )
    ...
    USER_ORDER_COMPLETED_NOTIFICATION = (
        "🎉 Ваш заказ «{item_name}» был выдан. Приятного пользования!"
    )
    EXPORT_STARTED = (
        "⏳ Начинаю сбор данных и создание архива... Это может занять некоторое время."
    )
    ...
    USERS_LIST_HEADER = (
        "📜 <b>Список пользователей (Страница {page}/{total_pages})</b>\n\n"
    )
    ...
    CHANGE_POINTS_PROMPT = (
        "Введите сумму баллов для начисления (например, 50) или списания (-50):"
    )
    CHANGE_POINTS_REASON_PROMPT = "Введите причину начисления/списания:"
    CHANGE_POINTS_SUCCESS = (
        "✅ Баланс пользователя {name} изменен. Новый баланс: {balance}"
    )
    ...
    USER_DEMOTED_VOLUNTEER_NOTIFY = (
        "⚙️ Ваша роль изменена на 'студент'. Меню волонтера больше недоступно."
    )
    ...
    USER_UNBLOCKED_NOTIFY = (
        "🎉 Ваш аккаунт был разблокирован. Теперь вы снова можете пользоваться ботом."
    )
    MANAGE_USER_REGS_HEADER = (
        "🎟️ Управление регистрациями для: <b>{name}</b>\n\nВыберите действие:"
    )
    MANUAL_REG_CHOOSE_EVENT = (
    ...
    MANUAL_CANCEL_NO_REGS = (
        "У этого пользователя нет активных регистраций на будущие мероприятия."
    )
    MANUAL_CANCEL_SUCCESS = "Регистрация пользователя {name} на {event_name} отменена."
    MANUAL_CANCEL_SUCCESS_NOTIFY = (
        "⚙️ Администратор отменил вашу запись на мероприятие «<b>{event_name}</b>»."
    )
    ...
    MANAGE_WAIVERS_WITH_WAIVERS = (
        "Нажмите на медотвод, чтобы удалить, или установите новый:"
    )
    ...
    MANUAL_WAIVER_NOTIFY = (
        "⚕️ Администратор установил вам медотвод до {date}.\n<b>Причина:</b> {reason}"
    )
    ...
    FEEDBACK_GET_WHAT_LIKED = (
        "Отлично! Расскажи, пожалуйста, что тебе понравилось больше всего?"
    )
    FEEDBACK_GET_WHAT_DISLIKED = (
        "Спасибо! А что можно было бы улучшить? Что не понравилось?"
    )
