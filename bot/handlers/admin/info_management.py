...
async def start_info_editing(
    ...
    if not sections:
        await callback.answer(
            "Информационные разделы не найдены в базе данных.", show_alert=True
        )
        return
...
@router.callback_query(
    EditInfoSection.choosing_section, F.data.startswith("edit_info_")
)
async def choose_section_to_edit(
    callback: types.CallbackQuery, state: FSMContext, session: AsyncSession
):
...
async def process_new_info_text(
