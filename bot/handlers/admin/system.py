    ...
    try:
        ...
        await bot.send_document(
            ...
            document=types.BufferedInputFile(
                xlsx_archive_bytes.read(), filename=filename
            ),
...
async def import_data_start(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        "Отправьте .xlsx файл для импорта/обновления данных пользователей. Обязательные колонки: `phone_number` (для поиска), `full_name`, `university`."
    )
    await callback.answer()
...
async def import_old_db_start(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        "Отправьте .xlsx файл для импорта старой базы данных."
    )
    await callback.answer()
...
async def process_import_old_db_file(
