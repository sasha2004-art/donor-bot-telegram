...
async def show_audience_choice_menu(message: types.Message, state: FSMContext):
    ...
    if current_filters:
        ...
        for key, value in current_filters.items():
            text_parts.append(
                f"  - {key.replace('_', ' ').capitalize()}: <code>{value}</code>"
            )
...
async def get_mailing_text(message: types.Message, state: FSMContext):
    ...
    await state.update_data(
        message_text=message.html_text, photo_id=None, video_id=None, filters={}
    )
    await state.set_state(Mailing.awaiting_media)
    await message.answer(
        Text.MAILING_STEP_2_MEDIA_PROMPT,
...
@router.callback_query(
    Mailing.awaiting_audience_choice, F.data.startswith("mail_audience_type_")
)
async def choose_audience_filter_type(
    ...
    if not items:
        await callback.answer(
            "Нет доступных значений для этого фильтра.", show_alert=True
        )
        return

    keyboard = inline.get_dynamic_mailing_filter_keyboard(
        items, filter_type, "mail_audience_back"
    )
...
@router.callback_query(
    Mailing.awaiting_audience_choice, F.data.startswith("mail_filter_")
)
...
async def back_to_audience_choice_menu(
...
@router.callback_query(
    Mailing.awaiting_audience_choice, F.data == "mail_audience_reset"
)
...
@router.callback_query(
    Mailing.awaiting_audience_choice, F.data == "mail_to_event_participants"
)
async def choose_event_for_mailing(
    callback: types.CallbackQuery, state: FSMContext, session: AsyncSession
):
    ...
    builder.row(
        types.InlineKeyboardButton(text="↩️ Назад", callback_data="mail_audience_back")
    )

    await callback.message.edit_text(
        "Выберите мероприятие, участникам которого нужно отправить рассылку:",
...
@router.callback_query(
    Mailing.awaiting_audience_choice, F.data == "mail_audience_finish"
)
async def finish_audience_selection(
    callback: types.CallbackQuery, state: FSMContext, session: AsyncSession
):
    ...
    preview_text = Text.MAILING_PREVIEW_HEADER.format(
        audience=audience_text, count=recipient_count
    )
    if photo_id:
        preview_text += Text.MAILING_PREVIEW_WITH_PHOTO
    if video_id:
        preview_text += Text.MAILING_PREVIEW_WITH_VIDEO
    ...
    await callback.message.answer(
        preview_text,
        reply_markup=inline.get_mailing_confirmation_keyboard(),
        parse_mode="HTML",
    )
    await callback.answer()
...
async def confirm_and_start_mailing(
...
async def _send_broadcast_safe(
    bot: Bot, user_id: int, text: str, photo_id: str | None, video_id: str | None
):
    ...
    try:
        ...
        logger.warning(
            f"Failed to send broadcast to user {user_id}. Bot was blocked or user deactivated."
        )
    except TelegramBadRequest as e:
        if "chat not found" in str(e):
            logger.warning(
                f"Failed to send broadcast to user {user_id}. Chat not found."
            )
        else:
...
async def do_mailing(
    ...
    async with async_session_maker() as session:
        try:
            ...
            tasks = [
                _send_broadcast_safe(
                    bot, user.telegram_id, message_text, photo_id, video_id
                )
                for user in users
            ]
            ...
            logger.info(
                f"Mailing finished. Sent: {success_count}, Failed: {fail_count}."
            )
