...
async def start_answering_question(
    ...
    question = await session.get(
        question_requests.Question,
        question_id,
        options=[joinedload(question_requests.Question.user)],
    )
    if not question:
    ...
    await state.update_data(
        question_id=question.id, user_to_answer_id=question.user.telegram_id
    )
...
async def process_answer(
    ...
    await question_requests.answer_question(
        session, question_id, message.text, admin_user.id
    )
