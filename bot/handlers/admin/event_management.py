...
async def process_event_location_text(
...
@router.callback_query(
    EventCreation.awaiting_blood_center, F.data.startswith("select_blood_center_")
)
async def process_blood_center_selection(
    ...
    await callback.message.edit_text(
        Text.EVENT_CREATE_STEP_5_TYPE, reply_markup=inline.get_donation_type_keyboard()
    )


@router.callback_query(
    EventCreation.awaiting_blood_center, F.data == "add_new_blood_center"
)
...
async def process_new_blood_center_name(
    message: types.Message, state: FSMContext, session: AsyncSession
):
...
@router.callback_query(
    EventCreation.awaiting_donation_type, F.data.startswith("settype_")
)
...
async def process_event_limit(
    ...
    try:
        ...
        blood_center = await admin_requests.get_blood_center_by_id(
        ...
        await message.answer(
            text, reply_markup=inline.get_event_creation_confirmation_keyboard()
        )
    except ValueError:
...
@router.callback_query(
    EventCreation.awaiting_confirmation, F.data == "confirm_create_event"
)
async def confirm_event_creation_and_notify(
    ...
    event_data["event_datetime"] = datetime.datetime.fromisoformat(
        event_data["event_datetime"]
    )
...
async def _send_notification_safe(bot: Bot, user: User, text: str, **kwargs):
    ...
    try:
        ...
        logger.warning(
            f"Failed to send notification to user {user.id}. Bot was blocked."
        )
    except TelegramBadRequest as e:
        if "chat not found" in str(e):
            logger.warning(
                f"Failed to send notification to user {user.id}. Chat not found."
            )
        else:
            logger.error(
                f"Failed to send new event notification to user {user.id}. Error: {e}"
            )
    except Exception as e:
        logger.error(
            f"Failed to send new event notification to user {user.id}. Unexpected error: {e}"
        )
    return False


async def send_new_event_notifications(
    event: Event, bot: Bot, status_message: types.Message
):
    """
    ...
    async with async_session_maker() as session:
        try:
            users_to_notify = await user_requests.get_users_for_event_notification(
                session, event
            )
            total_users = len(users_to_notify)
            logger.info(
                f"Starting mailing for event '{event.name}'. Found {total_users} users."
            )

            if total_users == 0:
                await status_message.edit_text(
                    "✅ Рассылка завершена. Подходящих пользователей для уведомления не найдено."
                )
                return
            ...
            await status_message.edit_text(
                Text.MAILING_FINISHED_NOTIFICATION.format(
                    success=success_count, fail=fail_count
                )
            )
        except Exception as e:
            logger.error(
                f"Critical error during new event mailing for event {event.id}: {e}",
                exc_info=True,
            )
...
async def view_active_events(callback: types.CallbackQuery, session: AsyncSession):
    ...
    await callback.message.edit_text(
        Text.ADMIN_CHOOSE_EVENT_TO_MANAGE, reply_markup=builder.as_markup()
    )
    await callback.answer()
...
async def show_single_event_card(callback: types.CallbackQuery, session: AsyncSession):
    ...
    donation_type_ru = Text.DONATION_TYPE_RU.get(
        event.donation_type, event.donation_type
    )

    feedback_count = await session.scalar(
        select(func.count(admin_requests.Feedback.id)).where(
            admin_requests.Feedback.event_id == event_id
        )
    )
    ...
    text = Text.EVENT_CARD_TEMPLATE.format(
        ...
        blood_center_name=(
            event.blood_center.name if event.blood_center else "Не указан"
        ),
    ...
    await callback.message.edit_text(
        text,
        reply_markup=inline.get_single_event_management_keyboard(
            event.id, event.registration_is_open, has_feedback=(feedback_count > 0)
        ),
        parse_mode="HTML",
    )
    await callback.answer()
...
async def toggle_event_registration(
    callback: types.CallbackQuery, session: AsyncSession
):
    event_id = int(callback.data.split("_")[-1])
    new_status = await admin_requests.toggle_event_registration_status(
        session, event_id
    )
    await session.commit()
    alert_text = (
        Text.EVENT_TOGGLE_REG_OPEN if new_status else Text.EVENT_TOGGLE_REG_CLOSED
    )
...
async def start_event_editing(
    ...
    fields = {
        ...
        "event_date": "Дата",
        "location": "Место",
        "blood_center_id": "Центр крови",
        "points_per_donation": "Баллы",
        "participant_limit": "Лимит",
    }
    builder = InlineKeyboardBuilder()
    for key, name in fields.items():
        builder.row(
            types.InlineKeyboardButton(
                text=f"Изменить: {name}", callback_data=f"edit_field_{key}"
            )
        )
    builder.row(
        types.InlineKeyboardButton(
            text="✅ Завершить", callback_data=f"admin_show_event_{event_id}"
        )
    )

    await callback.message.edit_text(
        Text.EVENT_EDIT_PROMPT.format(event_name=event.name),
...
async def choose_field_to_edit(
    ...
    if field_to_edit == "blood_center_id":
        ...
        await callback.message.edit_text(
            ...
            reply_markup=inline.get_blood_centers_keyboard(
                blood_centers, edit_mode=True
            ),
        )
    else:
        await state.set_state(EventEditing.awaiting_new_value)
        prompt = Text.EVENT_EDIT_FIELD_PROMPTS.get(
            field_to_edit, "Введите новое значение:"
        )
...
async def process_new_value_for_event(
    message: types.Message, state: FSMContext, session: AsyncSession
):
    data = await state.get_data()
    field, event_id, new_value_str = (
        data.get("field_to_edit"),
        data.get("event_id"),
        message.text,
    )
    ...
    try:
        if field == "event_date":
            update_value = datetime.datetime.strptime(new_value_str, "%d.%m.%Y %H:%M")
        elif field in ["points_per_donation", "participant_limit"]:
            update_value = int(new_value_str)
        else:
            update_value = new_value_str
    except ValueError:
    ...
    await state.clear()
    await message.answer(
        Text.EVENT_EDIT_SUCCESS, reply_markup=inline.get_back_to_admin_panel_keyboard()
    )


@router.callback_query(
    EventEditing.awaiting_new_value, F.data.startswith("select_blood_center_")
)
async def process_new_blood_center_for_event(
    callback: types.CallbackQuery, state: FSMContext, session: AsyncSession
):
    data = await state.get_data()
    ...
    await admin_requests.update_event_field(
        session, event_id, "blood_center_id", new_blood_center_id
    )
    await session.commit()
    await state.clear()
    await callback.message.edit_text(
...
@router.callback_query(
    EventEditing.awaiting_new_value, F.data == "add_new_blood_center_edit"
)
async def add_new_blood_center_for_edit(
    callback: types.CallbackQuery, state: FSMContext
):
...
async def process_new_blood_center_name_for_edit(
    message: types.Message, state: FSMContext, session: AsyncSession
):
    data = await state.get_data()
    event_id = data.get("event_id")
    ...
    await admin_requests.update_event_field(
        session, event_id, "blood_center_id", new_blood_center.id
    )
    await session.commit()
    await state.clear()
    await message.answer(
...
@router.callback_query(
    F.data.startswith("admin_event_participants_"), RoleFilter("admin")
)
async def get_event_participants(callback: types.CallbackQuery, session: AsyncSession):
    event_id = int(callback.data.split("_")[-1])
    event, participants_regs = await admin_requests.get_event_with_participants(
        session, event_id
    )
    if not event:
        return await callback.answer(Text.EVENT_NOT_FOUND, show_alert=True)
    if not participants_regs:
        return await callback.answer(
            Text.EVENT_NO_PARTICIPANTS.format(event_name=event.name), show_alert=True
        )
    ...
    file = types.BufferedInputFile(
        output.getvalue().encode("utf-8-sig"), filename=f"participants_{event.id}.csv"
    )
    await callback.message.answer_document(
        file, caption=Text.EVENT_PARTICIPANTS_CAPTION.format(event_name=event.name)
    )
    await callback.answer()


@router.callback_query(F.data.startswith("admin_cancel_event_"), RoleFilter("admin"))
async def ask_for_cancellation_confirmation(
    callback: types.CallbackQuery, session: AsyncSession
):
    event_id = int(callback.data.split("_")[-1])
    event = await event_requests.get_event_by_id(session, event_id)
    ...
    reg_count = await admin_requests.get_event_registrations_count(session, event_id)
    await callback.message.edit_text(
        Text.EVENT_CANCEL_CONFIRMATION.format(
            event_name=event.name, reg_count=reg_count
        ),
...
async def confirm_and_cancel_event(
    ...
    for reg in participants_regs:
        try:
            safe_event_name = Text.escape_html(event.name)
            safe_datetime = Text.escape_html(
                event.event_datetime.strftime("%d.%m.%Y в %H:%M")
            )
            text = Text.EVENT_CANCEL_NOTIFICATION_TEXT.format(
                event_name=safe_event_name, datetime=safe_datetime
            )
            await bot.send_message(
                chat_id=reg.user.telegram_id, text=text, parse_mode="HTML"
            )
            success_count += 1
        except Exception as e:
            fail_count += 1
            logger.error(
                f"Failed to send cancellation to user {reg.user_id} for event {event.id}. Error: {e}"
            )
    ...
    await session.commit()
    await callback.message.edit_text(
        Text.EVENT_CANCEL_SUCCESS_REPORT.format(
            event_name=event.name, success=success_count, fail=fail_count
        ),
...
async def view_event_feedback(callback: types.CallbackQuery, session: AsyncSession):
    ...
    if len(report) > 4000:  # Telegram message limit
        ...
        file = types.BufferedInputFile(
            report.encode("utf-8"), filename=f"feedback_{event_id}.txt"
        )
...
async def start_post_processing(
    ...
    if not past_events:
        await callback.answer(
            "Нет прошедших мероприятий для обработки.", show_alert=True
        )
        return
...
async def show_participant_marking_menu(
    message: types.Message, state: FSMContext, session: AsyncSession
):
    ...
    _, participants = await admin_requests.get_event_with_participants(
        session, event_id
    )
    if not participants:
        await message.edit_text(
            "На это мероприятие не было зарегистрировано участников.",
            reply_markup=inline.get_back_to_events_menu_keyboard(),
        )
        await state.clear()
        return
    ...
    await message.edit_text(
        ...
        reply_markup=inline.get_participant_marking_keyboard(
            event_id, participants, marked_donations, marked_dkm
        ),
    )


@router.callback_query(
    PostEventProcessing.choosing_event, F.data.startswith("post_process_event_")
)
async def choose_event_for_processing(
    callback: types.CallbackQuery, state: FSMContext, session: AsyncSession
):
...
@router.callback_query(
    PostEventProcessing.marking_participants, F.data.startswith("mark_participant_")
)
async def mark_participant(
    callback: types.CallbackQuery, state: FSMContext, session: AsyncSession
):
...
@router.callback_query(
    PostEventProcessing.marking_participants, F.data.startswith("finish_marking_")
)
async def finish_marking(
    callback: types.CallbackQuery, state: FSMContext, session: AsyncSession
):
    ...
    if not marked_donations:
        await callback.answer(
            "Ни один участник не отмечен как сдавший кровь.", show_alert=True
        )
        return

    await callback.message.edit_text(
        "⏳ Сохраняю данные... Это может занять некоторое время."
    )

    report_lines = []
    for user_id in marked_donations:
        is_dkm = user_id in marked_dkm
        success, message = await admin_requests.manually_confirm_donation(
            session, user_id, event_id, is_dkm
        )
    ...
    await callback.message.edit_text(
        final_report, reply_markup=inline.get_back_to_events_menu_keyboard()
    )
