...
from bot.db.models import Feedback, NoShowReport
...
async def show_events_for_registration(
    message: types.Message, session: AsyncSession, user_id: int
):
    """
...
async def show_profile_data(callback: types.CallbackQuery, session: AsyncSession):
    ...
    if last_donation:
        blood_center = (
            last_donation.event.blood_center_name
            if last_donation.event
            else "Не указан"
        )
    ...
    await callback.message.edit_text(
        text, reply_markup=inline.get_back_to_profile_menu_keyboard(), parse_mode="HTML"
    )
    await callback.answer()
...
async def show_donation_history(callback: types.CallbackQuery, session: AsyncSession):
    ...
    if not donations:
        await callback.message.edit_text(
            Text.NO_DONATION_HISTORY,
            reply_markup=inline.get_back_to_profile_menu_keyboard(),
        )
        return
...
async def process_event_registration(
    ...
    def get_reg_success_kbd(ev_id: int):
        ...
        builder.row(
            types.InlineKeyboardButton(
                ...
                callback_data=f"get_event_qr_{ev_id}",
            )
        )
        builder.row(
            types.InlineKeyboardButton(
                text="🗓️ Добавить в календарь", callback_data=f"add_to_calendar_{ev_id}"
            )
        )
        builder.row(
            types.InlineKeyboardButton(
                text="❌ Отменить мою регистрацию", callback_data=f"cancel_reg_{ev_id}"
            )
        )
        builder.row(
            types.InlineKeyboardButton(
    ...
    existing_registration = await event_requests.find_specific_registration(
    ...
    is_eligible, reason = await event_requests.check_registration_eligibility(
        session, user, event
    )
    if is_eligible:
        ...
        await callback.answer(
            Text.REGISTRATION_FAILED.format(reason=reason), show_alert=True
        )
...
async def send_waivers_menu(
    message_to_answer: types.Message, user_tg_id: int, session: AsyncSession
):
    ...
    if not all_waivers:
        ...
        if system_waivers:
            ...
            for waiver in system_waivers:
                text_parts.append(
                    Text.WAIVER_ITEM_FORMAT.format(
    ...
    text = "\n".join(text_parts)
    keyboard = inline.get_my_waivers_keyboard(
        user_waivers_exist=bool(user_created_waivers)
    )
...
async def show_my_waivers(callback: types.CallbackQuery, session: AsyncSession):
    try:
        ...
        logger.warning(
            "Could not delete message in show_my_waivers, it might have been deleted already."
        )
...
async def show_info_text(callback: types.CallbackQuery, session: AsyncSession):
    ...
    await callback.message.edit_text(
        text_to_show,
        reply_markup=inline.get_back_to_info_menu_keyboard(),
        parse_mode="HTML",
        disable_web_page_preview=True,
    )
    await callback.answer()
...
async def send_calendar_file(callback: types.CallbackQuery, session: AsyncSession):
    ...
    file_to_send = BufferedInputFile(
        file=ics_content.encode("utf-8"), filename=f"event_{event.id}.ics"
    )
...
@feedback_router.callback_query(
    FeedbackSurvey.awaiting_well_being, F.data.startswith("fb_wb_")
)
async def process_well_being(callback: types.CallbackQuery, state: FSMContext):
    ...
    if score <= 3:
        ...
        await callback.message.edit_text(
            Text.FEEDBACK_GET_ORGANIZATION_SCORE,
            reply_markup=inline.get_feedback_organization_keyboard(),
        )
...
@feedback_router.callback_query(
    FeedbackSurvey.awaiting_organization_score, F.data.startswith("fb_org_")
)
async def process_org_score(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        Text.FEEDBACK_GET_WHAT_LIKED, reply_markup=inline.get_feedback_skip_keyboard()
    )
    await state.set_state(FeedbackSurvey.awaiting_what_liked)
    await callback.answer()


async def skip_or_process_text(
...
@feedback_router.callback_query(
    FeedbackSurvey.awaiting_what_liked, F.data == "fb_skip_step"
)
async def process_what_liked(
    event: types.Message | types.CallbackQuery, state: FSMContext
):
    await skip_or_process_text(
        event,
        state,
        "what_liked",
        FeedbackSurvey.awaiting_what_disliked,
        Text.FEEDBACK_GET_WHAT_DISLIKED,
...
@feedback_router.callback_query(
    FeedbackSurvey.awaiting_what_disliked, F.data == "fb_skip_step"
)
async def process_what_disliked(
    event: types.Message | types.CallbackQuery, state: FSMContext
):
    await skip_or_process_text(
        event,
        state,
        "what_disliked",
        FeedbackSurvey.awaiting_other_suggestions,
        Text.FEEDBACK_GET_OTHER_SUGGESTIONS,
        inline.get_feedback_skip_keyboard(),
    )
...
@feedback_router.callback_query(
    FeedbackSurvey.awaiting_other_suggestions, F.data == "fb_skip_step"
)
async def process_other_suggestions(
...
async def start_asking_question(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        "Напишите ваш вопрос, и организаторы скоро на него ответят."
    )
    await callback.answer()
...
async def process_no_show_reason(callback: types.CallbackQuery, session: AsyncSession):
    ...
    await callback.message.edit_text(
        "Спасибо за ваш ответ! Это поможет нам в организации будущих мероприятий."
    )
    await callback.answer()
