...
class User(Base):
    ...
    category: Mapped[str] = mapped_column(
        String(50), default="student", server_default="student"
    )
    is_dkm_donor: Mapped[bool] = mapped_column(
    ...
    blocks_given: Mapped[List["UserBlock"]] = relationship(
        foreign_keys="UserBlock.admin_id", back_populates="admin"
    )
    blocks_received: Mapped[List["UserBlock"]] = relationship(
        foreign_keys="UserBlock.user_id", back_populates="blocked_user"
    )
...
class Event(Base):
    ...
    blood_center_id: Mapped[int] = mapped_column(
        ForeignKey("blood_centers.id"), nullable=True
    )
...
class EventRegistration(Base):
    ...
    survey_reminder_sent: Mapped[bool] = mapped_column(
...
class UserBlock(Base):
    ...
    blocked_user: Mapped["User"] = relationship(
        foreign_keys=[user_id], back_populates="blocks_received"
    )
    admin: Mapped["User"] = relationship(
        foreign_keys=[admin_id], back_populates="blocks_given"
    )
...
class Question(Base):
    ...
    status: Mapped[str] = mapped_column(
        String(50), default="unanswered", index=True
    )  # unanswered, answered
    ...
    answered_by_admin_id: Mapped[int] = mapped_column(
        ForeignKey("users.id"), nullable=True
    )

    user: Mapped["User"] = relationship(foreign_keys=[user_id])
    answered_by_admin: Mapped["User"] = relationship(
        foreign_keys=[answered_by_admin_id]
    )
