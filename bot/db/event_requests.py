...
async def get_active_events_for_user(
    ...
    """
    user_registrations_subquery = select(EventRegistration.event_id).where(
...
async def get_event_by_id(session: AsyncSession, event_id: int) -> Event | None:
    ...
    stmt = (
        select(Event)
        .options(joinedload(Event.blood_center))
...
async def get_today_event(session: AsyncSession) -> Event | None:
    ...
    stmt = select(Event).where(
        Event.is_active == True, func.date(Event.event_datetime) == today
    )
    result = await session.execute(stmt)
    return result.scalar_one_or_none()
...
async def check_registration_eligibility(
    ...
    active_waiver_stmt = (
        ...
        .where(MedicalWaiver.user_id == user.id, MedicalWaiver.end_date >= event_date)
    ...
    if active_waiver:
        return (
            False,
            f"У вас действует отвод до {active_waiver.end_date.strftime('%d.%m.%Y')}. Причина: {active_waiver.reason}.",
        )
    ...
    future_registrations = (
        (await session.execute(future_registrations_stmt)).scalars().all()
    )
    ...
    for reg in future_registrations:
        ...
        potential_waiver_end_date = (
            registered_event.event_datetime.date() + datetime.timedelta(days=interval)
        )

        if event_date <= potential_waiver_end_date:
            return False, (
                f"Запись невозможна. У вас запланирована донация "
    ...
    past_donations_stmt = select(func.count(Donation.id)).where(
        Donation.user_id == user.id, Donation.donation_type == donation_type
    )
    ...
    future_registrations_count = (
        await session.execute(future_registrations_stmt)
    ).scalar_one()
    ...
    if donation_type == "whole_blood":
        ...
        limit_reason = (
            f"Вы достигли годового лимита ({limit}) на сдачу цельной крови, "
            f"учитывая прошлые и запланированные донации."
        )
    elif donation_type in ["plasma", "platelets", "erythrocytes"]:
        limit = 12
        limit_reason = (
            f"Вы достигли годового лимита ({limit}) на сдачу компонентов, "
            f"учитывая прошлые и запланированные донации."
        )
...
async def add_event_registration(
    ...
    if event:
        reg_count_stmt = select(func.count(EventRegistration.id)).where(
...
async def find_specific_registration(
    session: AsyncSession, user_id: int, event_id: int
...
async def confirm_donation_transaction(
    ...
    russian_donation_type = Text.DONATION_TYPE_RU.get(
