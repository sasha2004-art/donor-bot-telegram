...
from .event_requests import (
    find_specific_registration,
    add_event_registration,
    confirm_donation_transaction,
)
import math
...
async def get_users_page(
    ...
    items_result = await session.execute(
        select(User).order_by(User.full_name).offset(offset).limit(page_size)
    )
...
async def update_user_field(
    session: AsyncSession, user_id: int, field_name: str, new_value: any
):
    """
    ...
    if not hasattr(User, field_name):
        logger.error(
            f"Attempted to update a non-existent field '{field_name}' for User."
        )
        return
...
async def block_user(session: AsyncSession, user_id: int, admin_id: int, reason: str):
    ...
    block_record = UserBlock(
        user_id=user_id, admin_id=admin_id, reason=reason, is_active=True
    )
...
async def unblock_user(session: AsyncSession, user_id: int):
    ...
    stmt = (
        update(UserBlock)
        .where(UserBlock.user_id == user_id, UserBlock.is_active == True)
...
async def update_event_field(
    session: AsyncSession, event_id: int, field_name: str, new_value: any
):
...
async def get_blood_center_by_id(
    session: AsyncSession, blood_center_id: int
) -> BloodCenter | None:
...
# --- Manual Waiver ---
async def create_manual_waiver(
...
async def get_all_donations(session: AsyncSession) -> list:
    stmt = (
        select(Donation)
        .options(joinedload(Donation.user), joinedload(Donation.event))
...
async def create_main_admin(
    session: AsyncSession, tg_id: int, tg_username: str, full_name: str
):
    new_admin = User(
...
async def update_main_admin_data(
    session: AsyncSession, tg_id: int, tg_username: str, full_name: str
):
    stmt = (
        update(User)
        .where(User.telegram_id == tg_id)
        .values(role="main_admin", telegram_username=tg_username, full_name=full_name)
    )
    await session.execute(stmt)
    await session.commit()
...
async def get_event_with_participants(session: AsyncSession, event_id: int):
    stmt = (
        select(Event)
        .options(selectinload(Event.registrations).joinedload(EventRegistration.user))
...
async def get_all_data_for_export(session: AsyncSession) -> dict:
    data_to_export = {
        ...
        "event_registrations": (await session.execute(select(EventRegistration)))
        .scalars()
        .all(),
        "donations": (await session.execute(select(Donation))).scalars().all(),
        "medical_waivers": (await session.execute(select(MedicalWaiver)))
        .scalars()
        .all(),
...
async def toggle_event_registration_status(
...
async def manually_register_user(
    ...
    waiver_stmt = select(MedicalWaiver).where(
        MedicalWaiver.user_id == user.id, MedicalWaiver.end_date >= event.event_date
    )
    active_waiver = (await session.execute(waiver_stmt)).scalar_one_or_none()
    if active_waiver:
        return (
            False,
            f"У пользователя действует отвод до {active_waiver.end_date.strftime('%d.%m.%Y')}.",
        )
...
async def manually_confirm_donation(
    session: AsyncSession, user_id: int, event_id: int, became_dkm_donor: bool
) -> tuple[bool, str]:
    """
