...
async def update_user_credentials(
    session: AsyncSession, user_id: int, new_tg_id: int, new_username: str | None
):
    # Обновить Telegram ID и username
    stmt = (
        update(User)
        .where(User.id == user_id)
        .values(telegram_id=new_tg_id, telegram_username=new_username)
    )
...
async def get_user_profile_info(session: AsyncSession, user_id: int) -> dict | None:
    ...
    stmt_waiver = (
        select(MedicalWaiver.end_date)
        .where(MedicalWaiver.user_id == user.id, MedicalWaiver.end_date >= today)
        .order_by(MedicalWaiver.end_date.desc())
        .limit(1)
    )
    ...
    total_donations = await session.scalar(
        select(func.count(Donation.id)).where(Donation.user_id == user.id)
    )
...
async def get_user_donation_history(
...
async def get_user_active_waivers(
    session: AsyncSession, user_id: int
) -> list[MedicalWaiver]:
...
async def get_users_for_event_notification(
    ...
    active_waiver_subquery = (
        ...
        .where(MedicalWaiver.end_date >= event_date_only)
        .distinct()
    )
    ...
    for user in initial_users:
        ...
        donations_stmt = (
            select(Donation)
            .where(Donation.user_id == user.id, Donation.donation_date >= one_year_ago)
            .order_by(Donation.donation_date.desc())
        )
        user_donations = (await session.execute(donations_stmt)).scalars().all()

        # --- Проверка 1: Годовые лимиты ---
        donations_of_event_type = [
            d for d in user_donations if d.donation_type == event.donation_type
        ]
        ...
        waiver_end_date_from_last = last_donation.donation_date + datetime.timedelta(
            days=interval
        )
        ...
        if event.donation_type == "whole_blood":
            donations_whole_blood = [
                d for d in user_donations if d.donation_type == "whole_blood"
            ]
            if donations_whole_blood:
                last_whole_blood_donation = donations_whole_blood[0]
                interval_whole = 90 if user.gender == "female" else 60

                waiver_end_date_from_last_whole = (
                    last_whole_blood_donation.donation_date
                    + datetime.timedelta(days=interval_whole)
                )
...
async def add_user_waiver(
    session: AsyncSession, user_id: int, end_date: datetime.date, reason: str
):
...
async def delete_user_waiver(
    session: AsyncSession, waiver_id: int, user_id: int
) -> bool:
    """
    Удаляет медотвод, если он был создан этим же пользователем.
    Возвращает True в случае успеха.
    """
    stmt = delete(MedicalWaiver).where(
...
async def get_unlinked_user_by_fio(
    session: AsyncSession, full_name: str
) -> User | None:
    """
    Ищет пользователя по ФИО среди тех, у кого telegram_id <= 0.
    """
    stmt = select(User).where(User.full_name == full_name, User.telegram_id <= 0)
    result = await session.execute(stmt)
    return result.scalar_one_or_none()
