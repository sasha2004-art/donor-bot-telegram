...
try:
    ...
    print(
        "Убедитесь, что скрипт 'populate_db.py' находится в корневой папке вашего проекта."
    )
    exit()
...
async def create_events(session: AsyncSession) -> tuple[list[Event], list[Event]]:
    ...
    logger.info(
        f"Создание {PAST_EVENTS_COUNT} прошедших и {FUTURE_EVENTS_COUNT} будущих мероприятий..."
    )
    past_events = []
    future_events = []
...
async def create_registrations_and_donations(
    session: AsyncSession, users: list[User], events: list[Event]
):
        ...
        for user in users:
            if random.random() < REGISTRATION_CHANCE:
                regs_to_create.append(
                    EventRegistration(user_id=user.id, event_id=event.id)
                )
