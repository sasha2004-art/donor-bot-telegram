...
async def test_block_and_unblock_user(session: AsyncSession):
    ...
    # 1. Подготовка
    admin = User(
        phone_number="+7_admin",
        telegram_id=999,
        full_name="Admin",
        university="TestUni",
    )
    target_user = User(
        phone_number="+7_target",
        telegram_id=123,
        full_name="Target",
        university="TestUni",
    )
    ...
    await admin_requests.block_user(
        session, target_user.id, admin.id, "Test block reason"
    )
...
async def test_user_can_delete_own_waiver_but_not_system(session: AsyncSession):
    ...
    # 1. Подготовка
    user = User(
        phone_number="+1", telegram_id=1, full_name="Waiver User", university="TestUni"
    )
    session.add(user)
    await session.commit()
    ...
    can_delete_own = await user_requests.delete_user_waiver(
        session, waiver_by_user.id, user.id
    )
    assert can_delete_own is True

    # Пытаемся удалить системный
    can_delete_system = await user_requests.delete_user_waiver(
        session, waiver_by_system.id, user.id
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"gender": "male", "id": 2, "tg_id": 2},
            [],
            [{"days_ago": 10, "duration": 30}],
            False,
        ),
        # Сценарий 3: Мужчина, сдача крови < 60 дней назад, НЕ должен попасть
        (
            {"gender": "male", "id": 3, "tg_id": 3},
            [{"type": "whole_blood", "days_ago": 45}],
            [],
            False,
        ),
        # Сценарий 4: Мужчина, сдача крови > 60 дней назад, должен попасть
        (
            {"gender": "male", "id": 4, "tg_id": 4},
            [{"type": "whole_blood", "days_ago": 70}],
            [],
            True,
        ),
        # Сценарий 5: Женщина, сдача крови 80 дней назад. Интервал 90 дней. 80+15=95 > 90. ДОЛЖНА ПОПАСТЬ.
        (
            {"gender": "female", "id": 5, "tg_id": 5},
            [{"type": "whole_blood", "days_ago": 80}],
            [],
            True,
        ),
        # Сценарий 6: Женщина, сдача крови 100 дней назад. ДОЛЖНА ПОПАСТЬ.
        (
            {"gender": "female", "id": 6, "tg_id": 6},
            [{"type": "whole_blood", "days_ago": 100}],
            [],
            True,
        ),
        # Сценарий 7: Сдача плазмы 10 дней назад. Интервал 14 дней. 10+15=25 > 14. ДОЛЖЕН ПОПАСТЬ.
        (
            {"gender": "male", "id": 7, "tg_id": 7},
            [{"type": "plasma", "days_ago": 10}],
            [],
            True,
        ),
        # Сценарий 8: Сдача компонентов > 14 дней назад, должен попасть
        (
            {"gender": "male", "id": 8, "tg_id": 8},
            [{"type": "plasma", "days_ago": 20}],
            [],
            True,
        ),
        # Сценарий 9: Мужчина, достиг лимита (5) по цельной крови, НЕ должен попасть
        (
            {"gender": "male", "id": 9, "tg_id": 9},
            [{"type": "whole_blood", "days_ago": d} for d in [70, 140, 210, 280, 350]],
            [],
            False,
        ),
        # Сценарий 10: Пользователь со старым медотводом, должен попасть
        (
            {"gender": "male", "id": 10, "tg_id": 10},
            [],
            [{"days_ago": 100, "duration": 30}],
            True,
        ),
    ],
)
async def test_get_users_for_event_notification(
    session: AsyncSession, user_data, donations, waivers, expected_in_list
):
    """
    ...
    for wav in waivers:
        waiver = MedicalWaiver(
            ...
            end_date=today
            - datetime.timedelta(days=wav["days_ago"])
            + datetime.timedelta(days=wav["duration"]),
            reason="test",
            created_by="system",
        )
        session.add(waiver)
    ...
    if expected_in_list:
        assert (
            user.id in user_ids_to_notify
        ), f"User {user.id} should be in the list but was NOT"
    else:
        assert (
            user.id not in user_ids_to_notify
        ), f"User {user.id} should NOT be in the list but was"
...
async def test_admin_create_manual_waiver(session: AsyncSession):
    ...
    # 1. Подготовка
    admin = User(
        phone_number="+1", telegram_id=101, full_name="Admin", university="TestUni"
    )
    user = User(
        phone_number="+2", telegram_id=102, full_name="User", university="TestUni"
    )
