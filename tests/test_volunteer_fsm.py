...
async def volunteer_user(session: AsyncSession) -> User:
    user = User(
        phone_number="+7-volunteer",
        telegram_id=1001,
        full_name="Волонтер Тестов",
        role="volunteer",
        university="TestUni",
    )
    session.add(user)
...
async def donor_user(session: AsyncSession) -> User:
    user = User(
        phone_number="+7-donor",
        telegram_id=2002,
        full_name="Донор Тестов",
        role="student",
        university="TestUni",
        gender="male",
        # points=0,
...
async def registration(
    session: AsyncSession, donor_user: User, today_event: Event
) -> EventRegistration:
...
async def test_volunteer_fsm_happy_path(
    ...
    mock_bot.download.return_value = AsyncMock(
        read=AsyncMock(return_value=b"fake_photo_bytes")
    )
    ...
    mocker.patch(
        "bot.handlers.volunteer.read_qr", new_callable=AsyncMock, return_value=qr_data
    )
    ...
    callback_start = MockCallbackQuery(
        data="confirm_donation_qr", from_user_id=volunteer_user.telegram_id
    )
    await start_qr_confirmation(callback_start, state)

    assert await state.get_state() == VolunteerActions.awaiting_qr_photo
    callback_start.message.edit_text.assert_called_once_with(
        Text.VOLUNTEER_SEND_QR_PROMPT
    )
    ...
    donation_record_query = await session.execute(
        select(Donation).where(
            Donation.user_id == donor_user.id, Donation.event_id == today_event.id
        )
    )
...
async def test_volunteer_fsm_qr_read_fails(mocker, session, volunteer_user: User):
    ...
    state = MockFSMContext()
    mocker.patch(
        "bot.handlers.volunteer.read_qr", new_callable=AsyncMock, return_value=None
    )
    mock_bot = Mock(spec=Bot)
...
async def test_volunteer_fsm_donor_not_registered(
    mocker, session, volunteer_user, donor_user, today_event
):
...
async def test_volunteer_fsm_wrong_day(
    mocker, session, volunteer_user, donor_user, future_event
):
