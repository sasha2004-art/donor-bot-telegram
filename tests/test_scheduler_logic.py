...
async def test_send_post_donation_feedback(session: AsyncSession, session_pool, mocker):
    ...
    user_to_ignore_old = User(
        phone_number="+2", telegram_id=102, full_name="Old Donor", university="TestUni"
    )
    user_to_ignore_duplicate = User(
        phone_number="+3", telegram_id=103, full_name="Duplicate", university="TestUni"
    )
    session.add_all([user_to_notify, user_to_ignore_old, user_to_ignore_duplicate])
    await session.commit()

    donation_to_notify = Donation(
    ...
    session.add_all(
        [donation_to_notify, donation_to_ignore_old, donation_to_ignore_duplicate]
    )
    await session.commit()

    # 2. Выполнение
    await send_post_donation_feedback(
        bot=mock_bot, session_pool=session_pool, storage=MemoryStorage()
    )
