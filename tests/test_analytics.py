...
async def test_get_churn_donors(session: AsyncSession):
    ...
    user3 = User(
        id=3,
        full_name="Multiple Donor",
        telegram_id=333,
        phone_number="333",
        university="Test",
    )
    donation3_1 = Donation(
        user_id=3,
        donation_date=datetime.date.today() - datetime.timedelta(days=200),
        donation_type="whole_blood",
        #points_awarded=10,
    ...
    session.add_all(
        [user1, donation1, user2, donation2, user3, donation3_1, donation3_2]
    )
...
async def test_get_survey_dropoff(session: AsyncSession):
    ...
    reg2 = EventRegistration(
        user_id=39,
        event_id=event1.id,
        registration_date=datetime.datetime.now() - datetime.timedelta(days=5),
    )

    # Пользователь 3: опрос не пройден
    user3 = User(
        id=40,
        full_name="Failed Survey User",
        telegram_id=400,
        phone_number="400",
        university="Test",
    )
