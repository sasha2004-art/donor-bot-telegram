...
def validate_telegram_data(auth_data: str) -> dict:
    ...
    try:
        ...
        logger.error(
            f"Telegram WebApp data validation failed: {e}. Raw auth_data: '{auth_data}'"
        )
        raise HTTPException(status_code=403, detail=str(e))


async def process_survey_rules(
    answers: SurveyAnswers, user_gender: str
) -> tuple[str, int | None, str]:
    # Абсолютные противопоказания
    if answers.age == "no":
        return ("temp_waiver", 365000, "Возраст менее 18 лет.")
    if (
        answers.has_hiv_or_hepatitis == "yes"
        or answers.has_cancer_or_blood_disease == "yes"
        or answers.has_chronic_disease == "yes"
    ):
        return (
            "temp_waiver",
            365000,
            "Абсолютное противопоказание (ВИЧ, гепатит, онкология, болезни крови, астма).",
        )
    ...
    if answers.health_issues_last_month == "yes" or answers.symptoms == "yes":
        return (
            "temp_waiver",
            30,
            "ОРВИ, грипп или ангина в течение последнего месяца.",
        )
    ...
    if answers.tattoo_or_piercing == "yes":
        return (
            "temp_waiver",
            120,
            "Наличие свежей татуировки/пирсинга (отвод на 4 месяца).",
        )
    ...
    if answers.analgesics_last_3_days == "yes" or answers.medication_last_72h == "yes":
        return (
            "temp_waiver",
            3,
            "Прием анальгетиков или других лекарств в последние 3 дня.",
        )
    ...
    if answers.diet_followed == "no":
        return (
            "ok",
            0,
            "Несоблюдение диеты. Рекомендуется перенести донацию.",
        )  # Нестрогое правило
    if answers.sleep_last_night == "no":
        return (
            "ok",
            0,
            "Недостаточный сон. Рекомендуется перенести донацию.",
        )  # Нестрогое правило
    if answers.smoking_last_hour == "yes":
        return (
            "ok",
            0,
            "Курение в течение последнего часа. Рекомендуется воздержаться.",
        )  # Нестрогое правило
...
async def initial_admin_and_texts_setup():
    ...
    async with async_session_maker() as session:
        ...
        if not users_exist:
            ...
            logger.warning(
                f"Users table is empty. Creating main admin with ID: {super_admin_id}"
            )
            try:
                ...
                logger.info(
                    f"Main admin user created successfully for TG ID {super_admin_id}."
                )
            except Exception as e:
        ...
        info_texts_in_db = (
            await session.execute(select(func.count(InfoText.section_key)))
        ).scalar()
        if info_texts_in_db == 0:
            ...
            texts_to_add = [
                InfoText(
                    section_key="prepare",
                    section_title="Как подготовиться?",
                    section_text=Text.INFO_PREPARE,
                ),
                InfoText(
                    section_key="contraindications",
                    section_title="Противопоказания",
                    section_text=Text.INFO_CONTRAINDICATIONS,
                ),
                InfoText(
                    section_key="after",
                    section_title="Что делать после?",
                    section_text=Text.INFO_AFTER,
                ),
                InfoText(
                    section_key="dkm",
                    section_title="О донорстве костного мозга (ДКМ)",
                    section_text=Text.INFO_DKM,
                ),
                InfoText(
                    section_key="mifi_process",
                    section_title="О донациях в МИФИ",
                    section_text=Text.INFO_MIFI_PROCESS,
                ),
                InfoText(
                    section_key="contacts",
                    section_title="Связаться с организаторами",
                    section_text=Text.INFO_CONTACTS,
                ),
            ]
...
async def submit_survey_logic(
    session: AsyncSession, payload: SurveyPayload
    ...
    try:
        ...
        logger.info(
            f"submit_survey_logic: Validation successful for user_tg_id: {user_tg_id}, username: {user_username}"
        )
    ...
    if not user and user_username:
        logger.warning(
            f"User with tg_id {user_tg_id} not found. Trying to find by username '{user_username}'."
        )
        found_users = await admin_requests.find_user_for_admin(session, user_username)
        if found_users:
            user = found_users[0]
            logger.info(
                f"Found user by username: '{user.full_name}'. Updating their tg_id from {user.telegram_id} to {user_tg_id}."
            )
    ...
    if not user:
        logger.error(
            f"submit_survey_logic: User with tg_id {user_tg_id} or username {user_username} not found in DB."
        )
        raise HTTPException(status_code=404, detail="User not found in DB")

    status, days, reason = await process_survey_rules(answers, user.gender)
    logger.info(
        f"submit_survey_logic: Survey rules processed. Status: {status}, Reason: {reason}"
    )

    logger.info(
        f"submit_survey_logic: Found user '{user.full_name}' (ID: {user.id}) in DB."
    )
    ...
    if status == "ok":
        ...
        logger.info(
            f"submit_survey_logic: Status is TEMP_WAIVER. Creating waiver for {days} days."
        )
        end_date = datetime.date.today() + datetime.timedelta(days=days)
        await admin_requests.create_manual_waiver(
            session, user.id, end_date, reason, admin_id=0
        )
...
async def submit_survey_endpoint(
    ...
    if chat_id and text:
        try:
            ...
            logger.error(
                f"Endpoint: FAILED to send message to chat_id {chat_id}. Error: {e}",
                exc_info=True,
            )
    else:
        logger.warning(
            "Endpoint: No chat_id or text returned from logic, nothing to send."
        )
...
async def lifespan(app: FastAPI):
    ...
    if app.state.ngrok_url:
        ...
        logger.error(
            "Could not get ngrok URL after several attempts. WebApp will not work."
        )
        dp["ngrok_url"] = None
