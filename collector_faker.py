...
try:
    ...
    print(
        "Убедитесь, что скрипт 'collector_faker.py' находится в корневой папке вашего проекта."
    )
    exit()
...
async def create_new_users(session: AsyncSession) -> list[User]:
    ...
    for i in range(NEW_USER_COUNT):
        ...
        faculty = (
            random.choice(FACULTIES_MIFI) if university == "НИЯУ МИФИ" else "Не указан"
        )
...
async def create_past_events(
    session: AsyncSession, blood_centers: list[BloodCenter]
) -> list[Event]:
...
async def create_history_for_new_users(
    session: AsyncSession, new_users: list[User], past_events: list[Event]
):
    ...
    segments = {
        ...
        "dkm_candidates": new_users[
            CHURN_DONOR_COUNT
            + LAPSED_DONOR_COUNT : -(ACTIVE_DONOR_COUNT + SURVEY_DROPOFF_COUNT)
        ],
        "active": new_users[
            -(ACTIVE_DONOR_COUNT + SURVEY_DROPOFF_COUNT) : -SURVEY_DROPOFF_COUNT
        ],
    ...
    for user in segments["churn"]:
        ...
        donation_date = datetime.date.today() - datetime.timedelta(
            days=random.randint(185, 300)
        )
    ...
    for user in segments["lapsed"]:
        ...
        last_date = datetime.date.today() - datetime.timedelta(
            days=random.randint(275, 400)
        )
        for i in range(num_donations):
            create_donation_entry(
                user,
                random.choice(past_events),
                last_date - datetime.timedelta(days=i * 90),
            )
    ...
    for user in segments["active"]:
        for i in range(random.randint(1, 6)):
            create_donation_entry(
                ...
                random.choice(past_events),
                datetime.date.today()
                - datetime.timedelta(days=random.randint(15, 90) * (i + 1)),
            )
    ...
    logger.info(
        f"Обработка {len(segments['survey_dropoff'])} 'потерянных' пользователей..."
    )
    for user in segments["survey_dropoff"]:
        surveys_to_add.append(
            Survey(
                user_id=user.id,
                passed=True,
                verdict_text="Противопоказаний не выявлено.",
...
async def main():
    ...
    async with async_session_maker() as session:
        ...
        if not existing_centers:
            ...
            centers = [
                BloodCenter(name="Центр крови ФМБА России"),
                BloodCenter(name="Центр крови им. О.К. Гаврилова"),
            ]
